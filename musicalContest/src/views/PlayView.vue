<template>
  ...
  <section
    ...
    class="bg-gradient-primary min-h-screen flex justify-center items-center p-4"
  >
    <div
      class="w-full max-w-xl gap-y-8 p-6 sm:p-8 bg-white rounded-2xl shadow-2xl flex flex-col justify-center"
    >
      <div class="flex flex-col justify-center gap-4 items-center text-center">
        <div
        ...
        <h3 class="text-2xl font-bold text-primary mb-1">¡Adivina la canción!</h3>
        <p class="text-gray-500">
          Escucha fragmentos de {{ gameStore.snippetDuration }} segundos y adivina el título correcto
        </p>
      </div>

      <div class="flex flex-col gap-y-4 bg-gray-50 p-6 rounded-xl">
  ...
  >
    <div
      class="w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col h-full md:h-auto max-h-[90vh]"
    >
      <!-- HEADER -->
      <div class="bg-gray-50 p-4 sm:p-6 border-b border-gray-100 flex justify-between items-center flex-wrap gap-4">
        ...
        <div class="bg-white border border-gray-200 py-2 px-4 flex items-center gap-2 rounded-md shadow-sm">
      ...
      <!-- CONTENT -->
      <div class="p-4 sm:p-6 flex flex-col gap-6 overflow-y-auto">
        <!-- PROGRESS -->
        <div class="flex flex-col gap-2">
          <span class="text-sm font-medium text-gray-500">
            Pregunta {{ gameStore.currentIndex + 1 }} de {{ gameStore.playableSongs }}
          </span>
        ...
        <!-- PLAYER -->
        <div
          class="bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl p-4 sm:p-6 flex flex-col items-center justify-center gap-6 border border-primary/10"
        >
          <!-- BUTTON PLAY -->
          ...
          <div
            ...
            class="flex items-center w-full gap-3 sm:gap-4 text-primary-dark bg-white/60 p-3 rounded-xl backdrop-blur-sm"
          >
            ...
            />
            <div class="font-mono text-sm font-bold w-10 sm:w-12 text-right">
          ...
          <div class="text-center">
            ...
            <p class="text-xl sm:text-2xl font-bold text-primary-dark" v-if="gameStore.songTitle">
              {{ gameStore.songTitle }}
            </p>
            <p class="text-md text-primary-light" v-if="gameStore.songSubtitle">
  ...
  <section
    ...
    class="bg-gradient-primary min-h-screen flex justify-center items-center p-6"
  >
    <div
      class="w-full max-w-4xl bg-white backdrop-blur-md rounded-3xl shadow-2xl p-6 sm:p-8 flex flex-col items-center gap-8"
    >
      <h1 class="text-3xl sm:text-4xl font-extrabold text-secondary animate-bounce text-center">
        ¡Juego terminado!
      </h1>

      <StatComponent
        :stat="String(gameStore.playerPoints)"
        title="Puntuación"
        class="bg-gradient-primary p-4 rounded-xl"
      />

      <p class="text-xl sm:text-2xl text-primary w-full sm:w-1/2 text-center mt-2">
        {{ funnyPhrase }}
      </p>

      <div class="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="bg-white rounded-xl p-5 shadow-md hover:scale-105 transition">
          <p class="text-lg text-accent">Resultado:</p>
          <p class="text-2xl font-bold text-accent">
            {{ gameStore.score }} / {{ totalQuestions }}
          </p>
        </div>

        <div class="bg-white rounded-xl p-5 shadow-md hover:scale-105 transition">
          <p class="text-lg text-highlight">Precisión</p>
          <p class="text-2xl font-bold text-highlight">{{ accuracyPct }}%</p>
        </div>

        <div class="bg-white rounded-xl p-5 shadow-md hover:scale-105 transition">
          <p class="text-lg text-secondary">Tiempo medio</p>
      ...
      <div class="flex flex-col sm:flex-row w-full gap-4 mt-4">
        <ButtonComponent
          ...
          icon="refresh"
          class="bg-white text-primary font-bold rounded-2xl flex-1 justify-center hover:scale-105"
          @click="handleReset"
        />
        ...
        <ButtonComponent
          ...
          icon="star"
          class="bg-gradient-primary text-white font-bold rounded-2xl flex-1 justify-center hover:scale-105"
          to="/ranking"
        />
      </div>
    </div>
